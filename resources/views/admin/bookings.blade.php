        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
            ...
            <div class="flex items-center gap-2 w-full sm:w-auto">
                <div class="relative flex-1 sm:w-64">
                    ...
                    <input type="text" placeholder="Search customer or car..." class="w-full pl-9 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-xs focus:outline-none focus:border-blue-500 transition-all">
                </div>
            </div>
        </div>
                            ...
                            <td class="px-5 py-3">
                                <div class="flex items-center gap-2 min-w-0">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-[9px] text-gray-500">{{ $booking->user->email }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-5 py-3">
                                <div class="min-w-0">
                                    <p class="text-xs font-bold text-gray-800">{{ $booking->car->brand }} {{ $booking->car->model }}</p>
                                    <p class="text-[10px] text-gray-500">{{ $booking->car->plate_number }}</p>
                                </div>
                            </td>
                            <td class="px-5 py-3">
                                <div class="flex items-center gap-2 text-xs">
                                    <div class="text-gray-600">
                                        <p class="font-bold leading-none">{{ \Carbon\Carbon::parse($booking->start_date)->format('M d') }}</p>
                                        <p class="text-[9px] text-gray-400">Start</p>
                                    </div>
                                    <i class="bi bi-arrow-right text-gray-300"></i>
                                    <div class="text-gray-600">
                                        <p class="font-bold leading-none">{{ \Carbon\Carbon::parse($booking->end_date)->format('M d') }}</p>
                                        <p class="text-[9px] text-gray-400">End</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-5 py-3 text-right">
                                <p class="text-xs font-bold text-gray-800 whitespace-nowrap">${{ number_format($booking->total_price, 2) }}</p>
                            </td>
