                        ...
                        <div class="modal-body p-4">
                            <div class="d-flex align-items-center gap-3 mb-4 bg-light p-3 rounded-4 border">
                                ...
                                <div>
                                    ...
                                    <span class="text-primary fw-bold small">${{ number_format($car->daily_rate, 2) }} / day</span>
                                </div>
                            </div>
...
{{-- 2. BOOKING HISTORY MODALS --}}
@php
    $loopBookings = $myBookings ?? $bookings ?? null;
@endphp

@if($loopBookings)
    @foreach($loopBookings as $booking)
                    ...
                    <div class="modal-body p-4">
                        <div class="text-center mb-4">
                            <div class="mb-2">
                                <div class="list-group-item bg-transparent d-flex justify-content-between py-3 border-0">
                                    <span class="text-muted small fw-bold text-uppercase" style="font-size: 10px;">Booking Status</span>
                                    
                                    @if($booking->status === 'confirmed')
                                        <span class="badge bg-success-subtle border border-success text-success fw-bold rounded-pill px-3">CONFIRMED</span>
                                    @elseif($booking->status === 'completed')
                                        <span class="badge bg-light border text-dark fw-bold rounded-pill px-3">COMPLETED</span>
                        ...
                        <div class="list-group list-group-flush border-top border-bottom mb-4">
                            ...
                            <div class="list-group-item bg-transparent d-flex justify-content-between py-3 border-0">
                                ...
                                @elseif($booking->payment && $booking->payment->payment_status === 'verifying')
                                    <span class="badge border fw-bold rounded-pill px-3" style="background-color: #e0f2fe; border-color: #bae6fd !important; color: #0369a1;">VERIFYING</span>
                                @elseif(($booking->payment && $booking->payment->payment_status === 'cancelled') || $booking->status === 'cancelled')
                                    <span class="badge bg-danger-subtle border border-danger text-danger fw-bold rounded-pill px-3">CANCELLED</span>
                                @else
                                    <span class="badge bg-warning-subtle border border-warning text-dark fw-bold rounded-pill px-3">PENDING</span>
                                @endif
                        ...
                        <div class="p-3 bg-primary-subtle border border-primary-emphasis rounded-4 d-flex align-items-start gap-3">
                            <i class="bi bi-info-circle-fill text-primary"></i>
                            <p class="small mb-0 text-primary-emphasis">For security reasons, plate numbers must match the vehicle registered in your app during pickup.</p>
                        </div>
                    </div>
                    
                    <div class="modal-footer border-0 pt-0 pb-4 px-4 d-flex flex-column gap-2">
                        @if(($booking->status === 'pending' || $booking->status === 'confirmed') && (!$booking->payment || $booking->payment->payment_status === 'pending' || $booking->payment->payment_status === 'cancelled'))
                            <a href="{{ route('payment.gcash', $booking->id) }}" 
                            class="btn btn-primary w-100 fw-bold py-2 d-flex align-items-center justify-content-center gap-2 shadow-sm" 
                            style="border-radius: 0.75rem;">
                                <i class="bi bi-wallet2"></i> Pay via GCash
                            </a>
                        @endif

                        @if($booking->status === 'pending')
                            <form action="{{ route('bookings.cancel', $booking->id) }}" method="POST" class="w-100 m-0 mt-2">
                                @csrf 
                                @method('DELETE')
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-0 shadow-lg" style="border-radius: 1.5rem;">
                    ...
                    <div class="modal-body p-4">
                        ...
                        <div class="bg-light rounded-4 p-3 mb-4 border border-dashed">
                            ...
                            <table class="table table-borderless align-middle mb-0">
                                ...
                                <tfoot class="border-top">
                                    <tr>
                                        ...
                                        <td class="pt-3 text-end fw-bold text-primary" style="font-size: 1.1rem;">
                                            ${{ number_format($booking->total_price, 2) }}
                                        </td>
                                    </tr>
                                </tfoot>
